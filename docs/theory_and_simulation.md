# KUKA KR20 约束 IK 持杯运输：系统实证研究与消融分析
...
本文方法从目标采样、约束 IK 规划到 MuJoCo 跟踪评估的整体流程见图 1。

![图 1. KUKA KR20 持杯运输的规划与仿真评估流程](../outputs/word_figures/fig01_method_pipeline.png)
...
> 45° 为本文设定的粗粒度运动学安全阈值，不是工业标准或通用倾洒界限。真实风险与液面高度、粘度、容器几何及加速度耦合 [Ibrahim, 2005; Faltinsen and Timokha, 2009]；因此本文另在 §4.4 和 §7 检查阈值边界与动态代理量。
...
> 表 1 的参数与实现来源：`src/planner.py` 和 `src/baseline_comparison.py`。

**设计原理**：
- **r_o** 使用叉积而非倾角本身，因为叉积同时编码了方向和偏离程度，梯度更平滑
- **r_c** 通过惩罚关节角变化量促进轨迹连续，相当于一阶平滑正则化
- **r_l** 将关节角向中心吸引，避免在极限位置丧失运动灵活性
- **r_s** 是**软屏障惩罚**（非硬约束），仅在倾角超过 45° 时激活，允许极少量的约束违规以换取整体规划的可行性

#### 3.2.1 orientation 残差的参数化选择

orientation 残差采用 $\hat{\mathbf z}_{cup}\times\hat{\mathbf z}_{world}$ 而非完整轴角（axis–angle）参数化。交叉积对小倾角变化平滑，易于数值求导，也不存在四元数的双覆盖歧义。其局限是当 $θ\rightarrow\pi$ 且杯体完全倒置时，交叉积再次趋近零，梯度无法表征巨大姿态误差。本任务的规划倾角不超过 $π/2$，因此该奇异情形不在本文实验范围内。

### 3.3 求解方法

本文使用 SciPy `least_squares` 的信赖域反射（trust-region reflective）算法求解，参数为 xtol = 1e-5、ftol = 1e-5、gtol = 1e-5 和 max_nfev = 120。

**收敛特性**：完整方法在 150 个目标中成功规划 148 个，每条 45 路径点轨迹的总函数评估次数平均为 176.86，平均规划时间为 0.182 s。单个路径点的 `least_squares` 调用上限为 max_nfev = 120，该上限不应与整条轨迹的累计 nfev 混淆。

> position_tolerance = 0.02 m（2 cm）是本文基准任务的成功判定规格，不代表通用工业装配公差。若目标应用为精密倾倒，建议收紧至 0.005 m，并重新评估 IK 可达率和仿真跟踪率。

### 3.4 轨迹生成

从初始位置 p_0 到目标 p_d 采用**任务空间直线插值和三次平滑步进（cubic smoothstep）时间参数化**：
...
其中，t 为归一化时间，s(t) 为三次时间缩放函数，p_0 和 p_d 分别为初始与目标末端位置。
...
> 表 2 的配置来源：`src/baseline_comparison.py` 中的 `ABLATION_VARIANTS`。

### 4.2 结果

**表 3. 默认权重下四种消融变体的规划与仿真结果**
...
**表 4. tilt_limit = 10° 时两种屏障配置的消融结果**
...
**对实践者的建议**：默认权重配置（7.5/0.90/18.0/0.08/0.025）可作为本文目标分布下的初始工作点；迁移到其他机器人、负载或容差时需重新扫描。
...
> 表 7 的采样区域来源：`src/enhanced_experiments.py` 中的 `sample_targets_expanded`。
...
其中，p_k 为第 k 个时刻的末端位置，v_k 和 a_k 分别为有限差分速度与加速度，Δt 为相邻样本的时间间隔，θ_k 为杯体倾角，ω_k 为倾角变化率。

### 7.3 结果

**表 9. 30 个目标的动态倾洒代理指标分布**
...
静态最大倾角与最大加速度的 Pearson 相关系数为 0.75，与 Jerk 及角速度的系数接近 1.00，但后者主要由一个 Jerk 为 110.9 m/s³ 的异常目标驱动。以水平半径中位数 1.033 m 分组后，近距组的平均最大加速度和 Jerk 分别为 1.282 m/s² 和 8.535 m/s³，高于远距组的 0.987 m/s² 和 1.034 m/s³。本节的 30 条轨迹均满足 45° 静态阈值且仿真成功，但动态指标仍标记出一个高 Jerk 样本，说明两类判据对该样本并不一致。

## 8. 收敛特性分析

### 8.1 求解器行为

下表基于 `full_method_results.csv` 中 150 次规划记录，其中每条轨迹包含 45 个路径点。

**表 10. 完整方法的求解次数与规划时间统计**

| 指标 | 最小值 | 均值 | P50 | P95 | 最大值 | 标准差 |
|---|---:|---:|---:|---:|---:|---:|
| 每条轨迹的总函数评估次数（nfev） | 140 | 176.86 | 180 | 191 | 213 | 12.41 |
| 总规划时间（s） | 0.117 | 0.182 | 0.182 | 0.217 | 0.266 | 0.021 |

> 数据来源：outputs/ablation_study/full_method_results.csv。

0.182 s 的平均轨迹规划时间折合约 4.05 ms/路径点。该延迟低于常见的上层轨迹规划周期，但高于 500 Hz 底层控制回路的 2 ms 周期。因此，当前实现应用于规划层（planner level）的批量轨迹生成，不应解读为可直接在 500 Hz 控制层（controller level）在线迭代。

### 8.2 失效模式分析

**IK 规划失效（2/150，1.3%）**：
- 观测：两条轨迹的终点位置误差分别为 87.3 mm 和 104.5 mm，超过 20 mm 成功阈值
- 边界：现有日志不能区分几何不可达、初值敏感性与求解器局部收敛，因此不对原因作单一归纳

**仿真跟踪失效（1/148，0.7% 可达目标）**：
- 观测：该轨迹的最终跟踪误差为 35.1 mm，最大倾角为 10.15°；失败由位置误差阈值触发，而非 45° 倾角阈值
- 边界：移除倾角屏障后仍为 147/150 次仿真成功，本实验不支持将该失败归因于 tilt_barrier 缺失
...
完整方法在 3 个随机种子上的仿真成功率分别为 98.0%、96.0% 和 100.0%，均值为 98.0% ± 1.63%（均值 ± 总体标准差）。数据来源为 `outputs/ablation_study/full_method_results.csv`。
...
- **Position actuator**：仿真中使用位置伺服，非力矩控制。该设置仅验证位置伺服假设下的运动学轨迹，可能低估真实机器人的控制挑战

#### 9.2.1 跨机器人迁移的初步讨论

本文的残差组合以 6-DOF KUKA KR20 为验证对象，不能直接推广到其他机构。对 7-DOF 冗余机械臂，可将 limit_weight 项改写为零空间正则化，并重新扫描姿态与连续性权重。对力矩控制机器人，需要替换 position actuator 假设，并先在 torque control 下重复 §4.2 的消融以检查结论稳定性。对移动机械臂，还需将 base_pose 纳入 IK 变量，并增加底盘稳定性残差。

### 9.3 结论

本文在 KUKA KR20 持杯运输任务下比较了软权重残差组合、消融变体与优先级 IK。完整方法的仿真成功率为 98.0%，纯位置变体的主要损失是 IK 可达率，而无倾角屏障变体在默认与 10° 阈值实验中均与完整方法同分。因此，本文的证据支持将该实现用作指定目标分布下的可复现比较基线，但不支持将五项残差解读为对所有工况均必需，也不能代替真实液体与力矩控制验证。
