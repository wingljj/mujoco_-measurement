...
本文统一管理 8 张图。所有图均由独立的 XeLaTeX/PGFPlots/TikZ 源文件构建：矢量主文件位于 `figures/pgfplots/build/`，供 Word 使用的 600 DPI PNG 位于 `outputs/word_figures/`。图内不嵌入图号或图题；中文图题应在 Word 中通过“插入题注”另行添加，以便自动编号和交叉引用。

如果当前 Word 版本能够稳定插入 PDF，优先使用 `figures/pgfplots/build/` 中的矢量 PDF；否则使用对应的 600 DPI PNG。两种格式内容相同，不应混用不同构建批次的文件。
...
1. 在正文指定位置选择“插入 → 图片 → 此设备”，优先选择对应 PDF；若 Word 的 PDF 支持不稳定，则选择对应 PNG。
