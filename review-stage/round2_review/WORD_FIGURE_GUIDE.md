...
| 图号 | 文件基名 | 成品尺寸 | 建议位置 | 题注正文（不含自动编号） |
|---|---|---:|---|---|
| 图1 | `fig01_method_pipeline` | 15 cm × 6 cm | 引言的研究目标与完整工作流之后 | 机械臂持经纬仪测站转移的规划、执行与到位测量流程 |
| 图2 | `fig02_workspace_multiview` | 15 cm × 6 cm | 第6节工作空间目标分布 | 五类目标测站在XY、XZ和YZ平面的空间分布 |
| 图3 | `fig03_transport_sequence` | 15 cm × 5.5 cm | 第7节轨迹生成与运输阶段说明 | 机械臂持经纬仪转移过程的五阶段姿态示意 |
| 图4 | `fig04_case_study` | 15 cm × 9 cm | 第7节代表性任务分析 | 代表性测站任务的轨迹、关节角、经纬仪倾角与逐路径点IK位置残差 |
| 图5 | `fig05_ablation_comparison` | 8 cm × 6 cm，单栏 | 第8节消融结果 | 四种残差配置的IK可达率与仿真成功率 |
| 图6 | `fig06_error_tilt_margin` | 15 cm × 6 cm | 第8节误差、倾角与姿态裕度经验累积分布 | 完整方法的位置误差、经纬仪最大倾角及相对10°阈值的姿态裕度 |
| 图7 | `fig07_dynamic_metrics` | 15 cm × 8 cm | 第10节规划轨迹动态代理结果 | 成功测站转移任务的运输动态扰动代理指标分布 |
| 图8 | `fig08_baseline_comparison` | 8 cm × 6 cm，单栏 | 第11节外部基线比较 | 本文方法与优先级IK在匹配测站目标上的位置误差—倾角对比 |
...
4. 在图片下方选择“引用 → 插入题注”，类别设为“图”，由 Word 自动生成 `图 N`；只粘贴题注正文，不要手工输入图号，也不要把图号或题注烘焙进图片。
